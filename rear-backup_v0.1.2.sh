...
#------------------------------------------------------------------------------------------------------------
 ...
 SSHFSUser=speefak					# TODO userinput / configfile
 SSHFSHost=192.168.1.x					# TODO userinput / configfile
 BackupFileSuffix="OS"					# TODO userinput / configfile
 BackupTargetDirRemoteHost="ReaR_Backups"		# TODO userinput / configfile
 BackupExcludeDir=" /mnt /media /home "			# TODO userinput / configfile
 RearOutputMode=ISO 					# TODO rear output for bootable USB drive incl backup tar only available for connected USB hardware

 RequiredPackets="rear syslinux syslinux-utils pigz locate"
...
usage() {
	...
	ürintf " -sshcc		=> SSH connection check\n"
...
load_color_codes () {
	...
	LYellowe='\033[0;33m'	&&	Yellow='\033[1;33m'
...
SSH_connection_check () {
	...
	if [[ ! $? == 0 ]]; then
		...
		sshfs -o allow_other,follow_symlinks,nonempty ${SSHFSUser}@${SSHFSHost}: ${SSHFSMountpoint}
		if [[ ! $? == 0 ]]; then
			printf " SSH connection ${SSHFSUser}@${SSHFSHost} established $CheckMark\n"
		fi
...
check_ISO_file () {									# usage: check_ISO_file <isofile.iso>
	...
	# check ISO file - updatedb for quick image locaion and | otherwise input must include absolute path cause file cmd causes error
	...
	if [[ -f $ISOFileAbsolutePath ]]; then
		printf " image exists $CheckMark ($ISOFileAbsolutePath)\n"
	else
		usage " file not found: $ISOFileAbsolutePath"
	fi

	FileProperties=$(file $(locate "$ISOFileAbsolutePath"))
	...
	if [[ -z $(grep "bootable" <<< $FileProperties)  ]]; then
		request_loop4 () {
			Request=
			printf " non bootable ISO file $ISOFileAbsolutePath"
			read -n1 -e -p " Are you sure to use this image (y/N)? " Request
			if   [[ "$Request" == [yY] ]]; then
				printf " non bootable ISO image accepted $CheckMark\n"
...
detect_and_select_usb_drives () {
	...
	DeviceListInfoRAW=$(lshw -class disk)
	DeviceListOS=$(df -h | grep -v "/media/")

	# detect usb devices and safe output to $DetectUSBDrivesResultFile
	SAVEIFS=$IFS
	IFS=$(echo -en "\n\b")
	...
	request_loop2 () {
		Request=
		read -n1 -e -p " ALL DATA ON THIS DEVICE WILL BE LOST ! Use this drive (y/N)? " Request
		if   [[ "$Request" == [yY] ]]; then
			printf ""
		elif [[ "$Request" == [nN]  ]]; then
			printf " USB drive creation process canceled\n"
...
create_USB_drive_from_ISO () {
	...
	umount -f ${USBTargetDevicePath}* 2> /dev/null
...
#------------------------------------------------------------------------------------------------------------
	...
	# create ReaR backup using ISO file ouput including systembackup if no external ISO file specified
	if [[ -z $ExternalISOFile ]]; then
...
#------------------------------------------------------------------------------------------------------------
	...
	if [[ -n $CreateUSBDrive ]] || [[ -n $ExternalISOFile ]]; then
		...
		SourceISO="'${RearOutputDir}'/'$RearOutputImageName'"
		# check for external ISO file
...
## Infosoure ISO
# https://askubuntu.com/questions/372607/how-to-create-a-bootable-ubuntu-usb-flash-drive-from-terminal/667224#667224
