...
#------------------------------------------------------------------------------------------------------------
 ...
 Version=$(cat $0 | grep -m1 version | sed 's/^.* : //')
...
SSH_connection_check () {
	...
	if [[ ! $? == 0 ]]; then
		usage " SSH connection failed (${SSHFSUser}@${SSHFSHost}) "
	else
...
check_ISO_file () {									# usage: check_ISO_file <isofile.iso>
	...
	if [[ -f $1 ]]; then
		...
		updatedb
		ISOFileAbsolutePath=$(locate -b "$1" | grep "\.iso$")

		# check for more than one matched ISO file, print selection for various iso files
		ISOFilesCount=$(grep -c . <<< $ISOFileAbsolutePath)
	...
	if [[ -f $ISOFileAbsolutePath ]]; then
		...
		usage " file not found: $1\n   use absolute path if file is located on networkshares \n"
	fi
