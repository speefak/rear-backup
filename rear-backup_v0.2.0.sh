...
# TODO userinput / configfile
 ...
 BackupExcludeDir=" $RearTmpDirOverrideDir /mnt /media /var/tmp /var/lib/rear/output "
...
#------------------------------------------------------------------------------------------------------------
check_for_required_rear_version () {
    local min_version="2.9"
    local current_version
    current_version=$(rear -V 2>/dev/null | awk '{print $2}' | cut -d'-' -f1)

    # Version vergleichen
    if [ "$(printf '%s\n' "$min_version" "$current_version" | sort -V | head -n1)" != "$min_version" ]; then
        echo "⚠️  Your ReaR version ($current_version) is older than $min_version."
        echo "Systemd library paths changed in Debian 13. Please update ReaR to at least version $min_version."
        exit 1
    else
        echo "✅ ReaR version check passed ($current_version >= $min_version)."
    fi
}
#------------------------------------------------------------------------------------------------------------
...
#------------------------------------------------------------------------------------------------------------
pause_services () {					# pause_services <-q|s>  # quit/start

	for i in $PauseServiceUnits ;do
		# stopping service
		if   [[ $1 == "-q" ]]; then
			printf " stopping service "$i" \n"
			systemctl stop "$i"
		# restart service 
		elif [[ $1 == "-s" ]]; then
...
#------------------------------------------------------------------------------------------------------------
	...
	if   [[ -n $CreateISOBackup ]]; then
		...
		# stopping system services, create backup, restart system services
		pause_services -q
		create_ISO_Backup
		pause_services -s
...
# 0.2.0 => adding version check
# 0.1.9 => adding service pause funcation for creating backup ( e.g. autofs pause )
...
#------------------------------------------------------------------------------------------------------------
# install working rear version from github rear repo
...
mv /var/tmp/build-rear-2.9-git.0.0.unknown/rear_*.deb dist/rear_2.9-git.0.0.unknown_amd64.deb $HOME

# workaround for older versions
check_for_required_objects () {
	# check for missing libaries on debian based operating systems
	if grep -qiE 'debian|ubuntu|mint' /etc/os-release; then
		printf " debian based OS detected, checking for shared objekt files\n"
		ldd_status_check () {
		# check for libsystemd-shared-*.so
		LDDStatus=$(ldd /usr/lib/x86_64-linux-gnu/systemd/libsystemd-core-*.so | grep "not found$")
		if [[ -n $(grep "not found" <<< $LDDStatus)  ]]; then
			printf " $LDDStatus\n"
			printf " create links: /usr/lib/x86_64-linux-gnu/systemd > /etc/ld.so.conf.d/libsystemd-core.conf"
			echo "/usr/lib/x86_64-linux-gnu/systemd" > /etc/ld.so.conf.d/libsystemd-core.conf
			printf " register objects\n"
			sudo ldconfig
			ldd_status_check
		else
			printf " all required shared objects detected\n"
		fi
		}
		ldd_status_check
	fi
